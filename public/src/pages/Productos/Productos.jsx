...
import { buttonContainer, inputContainer, inputText, crud, leftAligned, editButton, scrollableTable,
 ...
 const Productos = () => {
  ...
  useEffect(() => {
    setLoading(true)
    axios.get('http://127.0.0.1:8001/nodes/Producto')
      .then(response => {
  ...
  const submit = (event, id) => {
    ...
    if (id === 0) {
      axios.post("http://127.0.0.1:8001/create_producto", {
        nombre,
        descripcion,
        categoria,
        precio,
      ...
      axios.put(`http://127.0.0.1:8001/update_producto/${id}`, {
        nombre,
        descripcion,
        categoria,
        precio,
  ...
  const deleteData = (id) => {
    axios.delete(`http://127.0.0.1:8001/delete_producto/${id}`)
      .then(() => {
        fetchData()
      })
  }
